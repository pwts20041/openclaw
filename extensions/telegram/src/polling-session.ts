    ...
    try {
      ...
      if (!this.#cachedBotInfo) {
        ...
        try {
          ...
          // Non-fatal: if the eager init fails (e.g. network unavailable on first cold
          // start, or the timeout fired), the runner will call bot.init() itself with
          // its own retry logic under the watchdog. We skip caching and let the existing
          // code path handle it.
        } finally {
