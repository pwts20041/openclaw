...
description: "使用可选的 Diffs 插件将前后文本或统一补丁渲染为网关托管的 diff 视图、文件（PNG 或 PDF）或两者兼有。"
read_when:
...
`diffs` 是一个可选的插件工具，带有简短的内置系统引导和配套技能，可将变更内容转换为供智能体使用的只读 diff 制品。
...
1. 启用插件。
2. 使用 `diffs`，设置 `mode: "view"` 以画布优先流程查看。
3. 使用 `diffs`，设置 `mode: "file"` 以聊天文件投递流程查看。
4. 使用 `diffs`，设置 `mode: "both"` 同时获取两种制品。
...
- 当渲染的 diff 具有可展开的上下文数据时会出现展开控件，这在前后对比输入中是典型的。
...
- 对于敏感 diff，设置较短的 `ttlSeconds`。
- 在不需要时避免在 diff 输入中发送密钥。
- 如果你的频道会大幅压缩图片（例如 Telegram 或 WhatsApp），优先使用 PDF 输出（`fileFormat: "pdf"`）。
