...
function jointsFrom(r) {
  return r?.details?.result?.joints ?? [];
}
...
const main = async () => {
  ...
  const ok = /executed/i.test(moveText) && d0 > 0.1;

  if (ok) {
    console.log(`VERIFY_MOVJ_OK d0=${d0.toFixed(3)}`);
  } else {
    console.log(`VERIFY_MOVJ_FAIL d0=${d0.toFixed(3)} msg=${moveText.replace(/\s+/g, " ")}`);
  }
};
