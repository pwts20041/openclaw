...
export function createSessionsManageTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      // Self-session operations use deferred mode to avoid deadlock: the action
      // executes after the current run ends, not during the tool call.
      const isSelfSession = resolvedKey === effectiveRequesterKey;
